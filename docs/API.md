...
- 生成层：默认按问题意图使用不同的确定性中文模板；有 citations 且配置 `RAG_LLM_API_KEY` 后，才会把问题、意图、命中证据、结构化资料库上下文和确定性基线答案交给 OpenAI-compatible LLM 生成更自然的最终回答。`AUTH_MODE=demo` 下还需要显式设置 `ENABLE_DEMO_RAG_LLM=true`。
