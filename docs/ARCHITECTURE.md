...
`apps/api/src/rag/evaluation.ts` 提供固定评估集，覆盖单 ticker 查询、连续追问、风险查询、来源追溯、无资料边界和 LLM 幻觉回退。`npm run eval:rag --workspace @pit/api` 会构造合成资料库、执行这些问题并输出通过率、平均耗时、引用数量和失败原因；API 测试也会调用同一 runner，防止评估脚本和测试断言分叉。
