...
> 独立设计并开发基于私有资料库的投资研究 AI 工作台，支持多类型资料录入、AI 解析与人工确认、标的聚合及 `RAG + LLM` 连续问答；实现 Supabase Auth、`user_id` 数据隔离、RLS、私有图片存储、导出删除和成本控制，并通过 36 项 API 与 Harness 测试验证核心流程、超时重试、回答回退及双用户隔离场景。
