...
决策：无 citations 时不调用生成模型，直接返回确定性模板。有 citations 时，LLM 答案必须引用至少一个证据 ticker；若答案提到动作、日期/时间或来源，这些值必须能在资料库上下文或 citations 中找到。校验失败统一回退模板答案。

理由：该规则把“只能基于资料库回答”从 ticker 级扩展到投研资料最关键的事实字段，能减少自然语言生成层的细节幻觉。
...
决策：在现有 RAG 检索层之后增加可选 OpenAI-compatible LLM 生成层。系统先检索用户知识库并生成确定性基线答案，再把用户问题、问题意图、citations 和基线答案交给 LLM。LLM 只能基于这些证据回答，不能编造外部事实或投资建议。未配置 `RAG_LLM_API_KEY` 或 LLM 调用失败时，自动回退到确定性模板答案。公开 demo 模式默认保护上传、存储和解析能力；如需展示真实 RAG + LLM，可显式设置 `ENABLE_DEMO_RAG_LLM=true`，仍保留无引用不调用模型和 groundedness 回退。
